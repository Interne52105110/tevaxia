...
export function assertPropcalcApiInput(value: unknown, endpoint: PropcalcEndpoint): asserts value is PropcalcApiInput {
 ...
 for(const [key,val] of Object.entries(input)){
  ...
  if(['isPrimary','isFirstTime','isNew','frenchVatOnFullPrice','frenchMicroEligible','frenchNonProfessional'].includes(key)){if(typeof val!=='boolean')throw new RangeError(`${key} must be boolean`);continue;}
