...
describe('folio statement is a copy of reconciled records',()=>{
 ...
 it('refuses unsupported currency, broken precision and unbalanced records',()=>{expect(()=>prepareFolioStatement({...folio,currency:'USD'},[])).toThrow();expect(()=>prepareFolioStatement(folio,[{...line('a',.1,.01),line_ht:NaN}])).toThrow();expect(()=>prepareFolioStatement(folio,[{...line('a',.1,.01),line_ttc:.12}])).toThrow();expect(()=>prepareFolioStatement(folio,[{...line('a',.1,.01),line_ht:.001}])).toThrow();});
