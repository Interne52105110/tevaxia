...
export interface EnergyAuditReport {title:string;sections:{title:string;text?:string;rows?:{label:string;value:string}[]}[];sources:string[]}
...
export async function generateEnergyAuditPdf(report:EnergyAuditReport):Promise<Blob>{
 ...
 return pdf(<Document><Page size="A4" style={s.page}><PageHeader title={report.title} reference={ref}/>
  {report.sections.map((section,index)=><View key={index}><Text style={s.h} minPresenceAhead={35}>{section.title}</Text>
