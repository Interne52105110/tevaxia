  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-card-border bg-card p-4 shadow-sm">
        <div className="relative">
          <input
            ...
            placeholder="Commune, localite ou quartier..."
      ...
      <div className="rounded-xl border border-card-border bg-card p-4 shadow-sm space-y-3">
        <InputField label="Surface habitable" value={bien.surface} onChange={(v) => setBien({ ...bien, surface: Number(v) })} suffix="m2" min={10} max={500} />
        <InputField label="Chambres" value={bien.nbChambres} onChange={(v) => setBien({ ...bien, nbChambres: Number(v) })} min={0} max={10} />
        <InputField
          label="Etage"
          type="select"
          value={bien.etage}
        ...
        />
        <ToggleField label="Parking inclus" checked={bien.parking} onChange={(v) => setBien({ ...bien, parking: v })} hint="+4%" />
        <ToggleField label="Bien neuf (VEFA)" checked={bien.estNeuf} onChange={(v) => setBien({ ...bien, estNeuf: v })} />
      </div>
      ...
      {result ? (
        <div className="space-y-3">
          <div className="rounded-xl bg-gradient-to-br from-navy to-navy-light p-5 text-white text-center shadow-lg">
            <div className="text-xs text-white/60">Estimation centrale</div>
            ...
            <div className="mt-2 flex items-center justify-center gap-4 text-xs text-white/70">
              <div>
                <div className="text-white/40 text-[10px]">Basse</div>
                <div className="font-semibold">{formatEUR(result.estimationBasse)}</div>
              </div>
              <div className="h-6 w-px bg-white/20" />
              <div>
                <div className="text-white/40 text-[10px]">Haute</div>
          ...
          {result.ajustements.length > 0 && (
            <div className="rounded-xl border border-card-border bg-card p-4 shadow-sm">
              <h3 className="text-xs font-semibold text-navy mb-2">Ajustements</h3>
        ...
        <div className="rounded-xl border-2 border-dashed border-card-border p-8 text-center">
          <p className="text-sm text-muted">Selectionnez une commune pour estimer</p>
        </div>
      )}
    </div>
  );
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-muted">
            Estimez et comparez deux biens au Luxembourg
          </p>
        </div>

        {/* Side by side columns */}
        <div className="grid gap-6 lg:grid-cols-2">
          <BienColumn label="Bien A" bien={bienA} setBien={setBienA} result={resultA} />
          <BienColumn label="Bien B" bien={bienB} setBien={setBienB} result={resultB} />
        </div>

        {/* Comparison summary */}
        {comparison && resultA && resultB && (
          <div className="mt-8 rounded-2xl border border-card-border bg-card p-6 shadow-sm">
            <h2 className="text-base font-semibold text-navy mb-4 text-center">Comparaison</h2>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {/* Price comparison */}
              <div className="rounded-lg bg-background p-4 text-center">
                <div className="text-xs text-muted mb-1">Ecart de prix</div>
              ...
              <div className="rounded-lg bg-background p-4 text-center">
                <div className="text-xs text-muted mb-1">Ecart prix/m2</div>
              ...
              <div className="rounded-lg bg-background p-4 text-center">
                <div className="text-xs text-muted mb-1">Le plus cher</div>
                ...
                <div className="text-xs text-muted mt-1">
                  {comparison.moreExpensive !== "equal" && (
                    <>de {formatEUR(Math.abs(comparison.diffPrix))} de plus</>
                  )}
                </div>
              </div>
              ...
              <div className="rounded-lg bg-background p-4 text-center">
                <div className="text-xs text-muted mb-1">Confiance</div>
                <div className="flex justify-center gap-3">
                  <div>
                    <div className="text-[10px] text-muted">Bien A</div>
                  ...
                  <div>
                    <div className="text-[10px] text-muted">Bien B</div>
                    <span className={`text-sm font-semibold ${
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-card-border">
                    ...
                    <th className="px-3 py-2 text-right text-xs font-semibold text-navy">Bien A</th>
                    <th className="px-3 py-2 text-right text-xs font-semibold text-navy">Bien B</th>
                    <th className="px-3 py-2 text-right text-xs font-semibold text-muted">Ecart</th>
                  </tr>
                </thead>
                <tbody>
                  <tr className="border-b border-card-border/50">
                    <td className="px-3 py-1.5 text-muted">Prix de base /m2</td>
                  ...
                  <tr className="border-b border-card-border/50">
                    <td className="px-3 py-1.5 text-muted">Ajustements</td>
                  ...
                  <tr className="border-b border-card-border/50">
                    <td className="px-3 py-1.5 text-muted">Prix ajuste /m2</td>
                  ...
                  <tr className="border-b border-card-border/50">
                    <td className="px-3 py-1.5 text-muted">Estimation basse</td>
                  ...
                  <tr className="border-b border-card-border/50 bg-navy/5">
                    <td className="px-3 py-2 font-semibold text-navy">Estimation centrale</td>
                  ...
                  <tr>
                    <td className="px-3 py-1.5 text-muted">Estimation haute</td>
        ...
        <p className="mt-8 text-xs text-muted text-center leading-relaxed max-w-2xl mx-auto">
          Estimation indicative basee sur les donnees publiques de l'Observatoire de l'Habitat
          et les coefficients d'ajustement statistiques. Ne constitue pas une evaluation professionnelle.
        </p>
      </div>
    </div>
  );
